...
doubleSecond xs = [f x | x <- xs | f <- cycle [id, (*2)]]
...
sumDigits (x:xs) = sumDigits xs + if x < 10 then x else sumDigits (toDigits x)
...
isValid n 
    | sumDigits (doubleSecond (toDigitsRev n)) `mod` 10 == 0 = True
    | otherwise = False
